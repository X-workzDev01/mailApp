...
public class MailController {
	...
	public ModelAndView sendSingleSMS(@RequestParam("mobile") String mobileNumber,
		...
		if (Objects.nonNull(mobileNumber) && Objects.nonNull(message)) {
			...
			try {
				...
				if (Objects.nonNull(response)) {
					...
					return modelAndView.addObject("msg",
							"The " + status + " To: " + mobileNumber + " and The Message Id: " + messageId.toString());
				} else
		...
		if (messageId != null) {
			try {
				...
				if (Objects.nonNull(response)) {
					...
					return modelAndView.addObject("msg", "Fetched Delivery Reports for " + messageId + " The Mobile: "
							+ mobile + " And Status: " + status);
				} else {
					return modelAndView.addObject("msg",
							"Delivery Reports Not available for the Message Id " + messageId);
				}
			} catch (Exception e) {
				logger.error(e.getMessage(), e);
			}
